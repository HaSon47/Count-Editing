...
geom_transforms = transforms.Compose([
    ...
    transforms.RandomPerspective(distortion_scale=0.2, p=0.7, fill=0)
])
...
def get_dataset(args):
    ...
    mask_dir = os.path.join("Path/to/CE-130 dataset") # REPLACE THIS WITH YOUR PATH AFTER UNZIP all_fsc_phase2_V2.zip
    ...
    for name in filenames:
        ...
        # get mask_path and exam bbox
        img_folder = name[:-2]
        mask_folder_path = os.path.join(mask_dir, "train", img_folder)
        if not os.path.exists(mask_folder_path):
            mask_folder_path = os.path.join(mask_dir, "val", img_folder)

        anno_path = os.path.join(mask_folder_path, "annotation.json")
        with open(anno_path, "r") as f:
            anno = json.load(f)

        num_mask = len(os.listdir(mask_folder_path))//2 - 1
        if num_mask == 1:
            # chỉ có 1 mask thì dùng chính mask đó vậy
            mask_path = os.path.join(mask_folder_path, "mask_1.png")
            exam_bbox = anno["inpainted_bboxes"][0]
        else:
            # có nhiều mask hơn thì dùng mask của object tiếp theo
            ind = int(name[-1])
            if ind < num_mask:
                mask_path = os.path.join(mask_folder_path, f"mask_{ind+1}.png")
                exam_bbox = anno["inpainted_bboxes"][ind]
            else:
                mask_path = os.path.join(mask_folder_path, f"mask_{ind}.png")
...
def crop_and_adjust_bbox(input_image, gt_image, mask_image, x1, y1, x2, y2, resolution):
    ...
    mask_image = mask_image.crop((crop_x1, crop_y1, crop_x2, crop_y2))
    ...
    return input_image, gt_image, mask_image, (new_x1, new_y1, new_x2, new_y2), scaled_bbox

def extract_and_augment_subject(gt_image, mask_image, bbox, pg, pa):
    """
    - Crop subject theo bbox
    - Apply mask để loại bỏ background
    - Pad thành ảnh vuông với nền đen
    - Apply augmentation (geom + color)
    """
    x1, y1, x2, y2 = bbox

    # 1. Crop image + mask
    subject = gt_image.crop((x1, y1, x2, y2))
    mask = mask_image.crop((x1, y1, x2, y2))

    # Ensure mask là single channel (L)
    if mask.mode != "L":
        mask = mask.convert("L")

    # 2. Apply mask (background -> đen)
    subject_rgba = subject.convert("RGBA")
    mask_resized = mask.resize(subject.size)

    subject_rgba.putalpha(mask_resized)

    # Convert về RGB với nền đen
    black_bg = Image.new("RGB", subject.size, (0, 0, 0))
    subject = Image.composite(subject, black_bg, mask_resized)

    # 3. Padding thành vuông (nền đen)
    ...
    square.paste(subject, (paste_x, paste_y))
...
def preprocess(examples, image_processor, args):
    ...
    for img_p, inpaint_p, mask_p, box_p, exam_bbox in zip(examples["image_path"], examples["inpaint_path"], examples["mask_path"], examples["box_path"], examples["exam_bbox"]):
        # Load ảnh và metadata
        input_image = Image.open(img_p).convert("RGB")
        gt_image = Image.open(inpaint_p).convert("RGB")
        mask_image = Image.open(mask_p).convert("L")
        ...
        subject_image = extract_and_augment_subject(gt_image, mask_image, exam_bbox, pg, pa)

        # Crop và điều chỉnh bbox
        input_image, gt_image, mask_image, local_bbox, scaled_bbox = crop_and_adjust_bbox(
            input_image, gt_image, mask_image, x1, y1, x2, y2, args.resolution
        )
...
if __name__ == "__main__":
    ...
    class DummyArgs:
        def __init__(self):
            self.dataset_name = "Path/to/CEGEN_train" 
            self.resolution = 512
    ...
    if len(dataset) > 0:
        ...
        for i in range(num_samples_to_check):
            visualize_input(processed_dataset, index=i, save_dir="debug_outputs_4")
