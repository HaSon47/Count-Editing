'''
- phase 2 train từ đầu Denoising LoRA tiếp của sub att fill
- dùng fsc_dataloader_4: lấy mask examplar khác chỗ vùng cần chèn
'''
print("run train_fsc_v3_phase2_ver2_changeMask.py")
import sys,os
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--work_dir",type=str,default="output/train_result_phase2_maskobjV2_e4",)
    # Tên của LoRA SAU KHI LƯU (Fine-tuned)
    parser.add_argument("--output_denoising_lora",type=str,default="subject_fill_union",) 
    
    parser.add_argument("--pretrained_condition_lora_dir",type=str,default="ckpt/Condition_LoRA",)
    parser.add_argument("--pretrained_subject_condition_lora_dir", type=str, required=True, help="path to fine-tune subject condtion lora from train_add_condtion phase.")
...
def main(args):
    ...
    for condition_type in lora_names:
        if condition_type == "subject":
            logger.info("load new weight Condition Subject LoRA")
