...
def parse_args(input_args=None):
    ...
    parser.add_argument("--condition_lora_dir",type=str,default="ckpt/ConditionAdd",)
    parser.add_argument("--denoising_lora_dir",type=str,default="ckpt/DenoisingAdd",)
    ...
    parser.add_argument("--work_dir",type=str,default="output_add",)
...
def get_inputs(test_dir):
    ...
    for img in os.listdir(img_test_dir):
        ...
        exam_mask_path = os.path.join(test_dir, "mask", img)
        ## get prompt
...
def inference(args):
    ...
    for condition_type in args.condition_types:
        if condition_type == 'subject':
            transformer.load_lora_adapter(f"{args.condition_lora_dir}/{condition_type}/pytorch_lora_weights.safetensors", adapter_name=condition_type)
        else:
